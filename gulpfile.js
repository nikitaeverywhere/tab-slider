...
gulp.task("move-files", () =>
  gulp
    .src([`src/options.html`, `src/options.css`])
...
gulp.task("move-background-script", () =>
  gulp
    .src(`src/*.js`)
    .pipe(
      rollup(
...
gulp.task(
  "build",
  gulp.series(
    gulp.parallel("log", "move-files", "move-images", "move-background-script"),
    "move-manifest",
    "zip"
  )
);
...
gulp.task(
  ...
  gulp.series(
    ...["clean", "build"].concat(
      browsers
        .slice(1)
        .reduce((acc, a) => acc.concat(["next-browser", "build"]), [])
    )
  )
);
